...
//			if(data != null && data.length > 0 && listData && TimeCompare.compareHourTime(data[1]) > 1){
	...
	public static void requestByPost(Context context,
			final ObserverCallBack callBack, String uri, Map<String, String> map,
			boolean isCache, boolean isShowDialog) {
...
//		requestUrl = requestUrl.substring(0,requestUrl.length()-1);
	...
	public static void requestSimpleByPost(Context context,
			final ObserverCallBack callBack, String uri, Map<String, String> map) {
...
//				httpClient.getCookieStore().getCookies().g
				...
				if(!isCache){
					//AnsynHttpRequest.sendBroadcastReceiverMessage(context, "11111111111");
				}
				data = null;
			}
		} catch (Exception e) {
